<?php
// CNN News US Edition RSS converter
...
for($i = 0; $i < 50; $i++){
    if(@$xml->channel->item[$i]->title>'0'){
        ...
        if(@$og['title']>'0'){
            ...
            $published = "1";
            $section = "News";
            $section_image = "images/news.png";
            $section_color = "1d1d1d";
            ...
            if($test==0){
                $connect->query("INSERT INTO `folder_items` (`item_token`,`item_url`,`item_name`,`item_description`,`item_icon`,`tags`,`section`,`section_image`,`section_color`) VALUES('".mysqli_real_escape_string($connect,$token)."','".mysqli_real_escape_string($connect,$url)."','".mysqli_real_escape_string($connect,$title)."','".mysqli_real_escape_string($connect,$description)."','".mysqli_real_escape_string($connect,$image)."','".mysqli_real_escape_string($connect,$tags)."','".$section."','".$section_image."','".$section_color."');");
            }
        }
    }
}
